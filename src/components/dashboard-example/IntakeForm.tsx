...
export function IntakeForm() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      metrics_challenge:
        (fd.get("metrics_challenge") as string)?.trim() || undefined,
    };
    try {
      const res = await fetch("/api/good-agency-dashboard-example/intake", {
        method: "POST",
      ...
      router.push(`/good-agency-dashboard-example/thanks/${json.id}`);
    } catch {
    ...
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <Field label="Your role" required>
        <input
          ...
          type="text"
          required
          placeholder="Founder / CEO, Integrator, Director of Revenue…"
          className={inputClass}
        />
      </Field>
      <Field label="What's your biggest metrics challenge right now? (optional)">
        <textarea
          name="metrics_challenge"
          rows={3}
          placeholder="Too many metrics, no clear owner, nobody reviews the dashboard weekly, metrics don't predict outcomes…"
          className={inputClass}
        />
      </Field>
      ...
      <Button type="submit" variant="cta" size="lg" disabled={submitting}>
        {submitting ? "Preparing your download…" : "Download the Dashboard Example"}
      </Button>
    </form>
  );
}
